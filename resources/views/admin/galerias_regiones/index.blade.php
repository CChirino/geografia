...
<table class="table-auto">
    <thead>
      <tr>
        <th class="px-8 py-2">Imagenes</th>
        <th class="px-8 py-2">Region o Estado</th>
        <th class="px-8 py-2" colspan="3">Transacciones</th>
      </tr>
    </thead>
    <tbody>
        @foreach($region as $r)
      <tr>
        <td class="border px-12 py-2">
          <img src="{{asset('storage/'.$r->imagenes) }}" width="200">
        </td>
        <td class="border px-12 py-2">{{$r->nombre_region}}</td>
        {{-- <td class="border px-8 py-2">
          <button class="bg-blue-300 hover:bg-blue-400 text-blue-800 font-bold py-2 px-4 rounded inline-flex items-center">
            ...
            <span><a href="{{ route('galerias-paises.show',$p->id) }}"> Ver</a></span>
          </button>
        </td>
        <td class="border px-8 py-2"><a href="">
            ...
            <span><a href="{{ route('galerias-paises.edit',$p->id) }}"> Editar</a></span>
          </button>
        </td> --}}
        <td class="border px-12 py-2">
          <form action="{{ route('galerias-paises.destroy',$r->id) }}" method="post">
            @csrf
            @method('DELETE')
