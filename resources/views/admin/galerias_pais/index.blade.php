...
<table class="table-auto">
    <thead>
      <tr>
        <th class="px-8 py-2">Bandera</th>
        <th class="px-8 py-2">Pais</th>
    ...
    <tbody>
      ...
      <tr>
        <td class="border px-12 py-2">
          <img src="{{asset('storage/'.$p->imagenes) }}" width="200">
        </td>
        <td class="border px-12 py-2">{{$p->nombre}}</td>
        ...
        <td class="border px-12 py-2">
          <form action="{{ route('galerias-paises.destroy',$p->id) }}" method="post">
            @csrf
            @method('DELETE')
